...
class MergerDetailViewController: CommonDataDetailViewController,UITableViewDelegate,UITableViewDataSource {

    var titiArr:[String] = [String]()
    var valueArr:[String?] = [String?]()
    var enterpriseModelArr:[ParticipateEnterpriseViewModel] = [ParticipateEnterpriseViewModel]()
    var viewModel:MergerViewModel = MergerViewModel(){
        didSet{
            ...
            titiArr = ["标的企业","并购时间","转让股份","并购金额"]
            fillValueArr()
            unfoldModel.showStr = viewModel.model.desc
            fillEnterpriseViewModelArr()
    ...
    func fillValueArr(){
        ...
        valueArr.append(viewModel.model.storkRight)
        valueArr.append(viewModel.model.amount)
    }
    
    func fillEnterpriseViewModelArr(){
        if viewModel.model.buyEnterpriseList == nil {
            return
        }
        for i in 0..<viewModel.model.buyEnterpriseList!.count{
            let particeModel = ParticipateEnterpriseViewModel()
            particeModel.model = viewModel.model.buyEnterpriseList![i]
            enterpriseModelArr.append(particeModel)
        }
    ...
    override func getData(_ isFooterRefresh:Bool){
        ...
        DataListManager.shareInstance.getMergerDetail(id, success: { [weak self](successTuple) in
            if successTuple.code == 0{
                ...
                if let dataDic = data{
                    ...
                    let model = MergerDataModel.objectWithKeyValues(dataDic as NSDictionary) as! MergerDataModel
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.navigationItem.title = "并购事件详情"
    ...
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        if section == 1 {
            return enterpriseModelArr.count
        }
        return 0
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 1 {
            let identity = "labelValueCell"
            let cell = ParticipateEnterpriseCell(style: .default, reuseIdentifier: identity)
            cell.viewModel = enterpriseModelArr[indexPath.row]
            cell.indexPath = indexPath
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        if indexPath.section == 1 {
            return enterpriseModelArr[indexPath.row].cellHeight!
        }
        return 44
    }
    ...
    func tableView(_ tableView: UITableView, heightForHeaderInSection section: Int) -> CGFloat {
        ...
        if section == 1 && self.enterpriseModelArr.count != 0{
            return 44
        }
        return 0.001
    }
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        if section == 1 && enterpriseModelArr.count>0{
            let label = UILabel()
            label.frame = CGRect(x: leftStartX, y: 0, width: self.view.frame.size.width, height: 40)
            label.font = UIFont.systemFont(ofSize: 15)
            label.text = "参与企业(\(enterpriseModelArr.count))"
            label.textColor = MyColor.colorWithHexString("#333333")
            lineStartH = 44 - 0.5
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.section == 1 {
            let model = self.enterpriseModelArr[indexPath.row]
            let vc = EnterpriseDetailViewController()
            if model.model?.id != nil {
                vc.id = (model.model?.id)!
...
class ParticipateEnterpriseViewModel: CommonViewModel {
    var model:BuyEnterpriseDataModel?
}
