...
//            //为了发送一条申请查看请求的时候 不显示时间
//            NSDictionary *ext = message.ext;
//            NSString *applyCheck = ext[applyCheckMessage];
//            NSString *postId = ext[postMessageId];
//            if (!([applyCheck isEqualToString:@"1"]&&[postId isEqualToString:[Account sharedOne].user.huanXinId])) {
//                
//            }
...
- (void)_sendMessage:(EMMessage *)message
{
    if (self.conversation.type == EMConversationTypeChat){
        ...
        if (extM == nil) {
            extM = [[NSMutableDictionary alloc]init];
        }
        NSString *name = [Account sharedOne].user.realName;
        name = name==nil?@"":name;
        NSString *image = [Account sharedOne].user.avatar;
        image = image==nil?@"":image;
        [extM setValue:name forKey:postMessageName];
        [extM setValue:image forKey:postMessageImg];
        [extM setValue:self.userName forKey:reciveMessageName];
        [extM setValue:self.userImg forKey:reciveMessageImg];
        [extM setValue:Account.sharedOne.user.huanXinId forKey:postMessageId];
        message.ext = extM;
    }
